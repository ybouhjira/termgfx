...
/// Color information with hex and RGB values
...
/// A palette containing a collection of colors
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Palette {
    pub name: String,
...
pub fn show_palette(palette: &Palette) {
    ...
    for (idx, color) in palette.colors.iter().enumerate() {
        ...
        // Show colored swatch using the color itself
...
mod tests {
    ...
    fn test_palette_not_found() {
        let palette = get_palette("nonexistent");
    ...
    #[test]
    fn test_case_insensitive_palette_lookup() {
        let palette1 = get_palette("DEFAULT");
        let palette2 = get_palette("default");
        assert!(palette1.is_some());
        assert!(palette2.is_some());
        assert_eq!(palette1.unwrap().name, palette2.unwrap().name);
    }

    #[test]
    fn test_export_palette_json() {
        let palette = get_palette("default").unwrap();
    ...
    #[test]
    fn test_all_palettes_accessible() {
        let palettes = get_all_palettes();
        assert!(palettes.len() >= 7, "Should have at least 7 palettes");
