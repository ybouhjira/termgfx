//! SVG export builder for creating scalable vector graphics output

use super::ExportConfig;
use std::io::Write;

/// SVG builder for programmatically creating SVG output from terminal graphics
...
impl SvgBuilder {
    /// Create a new SVG builder with default configuration
    ...
    /// Add a styled box (like termgfx box command output)
    ...
    pub fn add_bar_chart(&mut self, x: f32, y: f32, values: &[(String, f32)], max_value: f32) {
        ...
        for (idx, (label, value)) in values.iter().enumerate() {
            ...
            self.add_text(bar_x + bar_width / 2.0 - 20.0, y + bar_height + 25.0, label, "#333333", 12);
            self.add_text(bar_x + bar_width / 2.0 - 15.0, bar_y - 10.0, &value.to_string(), "#1e40af", 11);
        }
    }
    ...
    pub fn add_progress_bar(&mut self, x: f32, y: f32, width: f32, height: f32, percent: f32, style: &str) {
        ...
        self.add_text(x + width / 2.0 - 15.0, y + height / 2.0 + 5.0, &format!("{}%", percent as i32), "#ffffff", self.config.font_size);
    }
...
"#,
            ...
            self.config.width, self.config.height, self.config.background
        );
    ...
    /// Write the SVG output to a writer (e.g., file or stdout)
...
mod tests {
    ...
    fn test_svg_add_bar_chart() {
        let config = ExportConfig::default();
        let mut builder = SvgBuilder::new(config);
        let data = vec![("A".to_string(), 10.0), ("B".to_string(), 20.0), ("C".to_string(), 15.0)];
        builder.add_bar_chart(50.0, 50.0, &data, 25.0);
        assert!(builder.content.len() >= 9);
    }
}
